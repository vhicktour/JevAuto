...
/** A drag moves in a straight line at the drag's own pace (Cua drags take 500 ms by default), not a hand's arc. */
export function planDrag(from: Point, to: Point, durationMs = 500): Travel {
  return { from, to, control: { x: (from.x + to.x) / 2, y: (from.y + to.y) / 2 }, durationMs }
}
