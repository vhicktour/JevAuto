...
## Remaining phases (Victor, 2026-09-24: "finish all phases and fix bugs later")

10. **Phase 1b finish**: `src/main/settings.ts` (settings.json: watch, model, excluded apps; validated), `src/main/secrets.ts` (provider keys encrypted with `safeStorage` in `keys.bin`, write-only from the UI; they override `.env.local`), a Settings view in the activity window, onboarding buttons that open the Accessibility and Screen Recording panes, and "Copy diagnostics" (no keys, no screenshots).
11. **Phase 2 finish**: Jev in shadow mode on web steps (the web page's controls as the Choice; the frontier model's pick logged beside Jev's, never acting), five more web fixture tasks, and three read-only real-site tasks.
12. **Phase 4**: speed modes (Instant, Balanced, Cinematic), a teaching mode that draws trails, a live thumbnail of a hidden target on the island, and the island timeline.
13. **Phase 5**: electron-updater (ZIP + latest-mac.yml), a release script that notarizes when the `jevauto-notary` profile exists, and the final whole-branch review.
